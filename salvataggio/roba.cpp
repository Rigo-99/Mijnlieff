...
void Info_parziali(int grado){
	if (grado == grado_show){
		for(int i=0; i<25; i++){
			cerr << "Fin'ora ho salvato " << nodi[i] << " + " << copie[i] << " \tesiti a profondità " << i << "\tcon efficienza: " << media[i]/numeri[i] << endl;

		}
		cerr << "----- Time: " << time(NULL) - inizio << " s. Salvati " << n_tot << " nodi in "<< M <<" chiavi -----" << endl;
		
		tempi.push_back(time(NULL) - inizio);
		n_nodi.push_back(nodi[grado_save_max]);
		Stampa_tempi();
	}
}
...
bool da_cercare(int grado){
	if (grado >= grado_save_min){
		if (grado < grado_save_max + grado_diff_ricerca){
			return true;
		}
	}
	return false;
...
bool da_cercare(int grado){
	return da_salvare(grado);
}

bool da_salvare(int grado){
	if (grado <= grado_save_max){
		if(grado >= grado_save_min){
			if((grado-grado_save_min)%grado_save_mod == 0){
				return true;
			}
		}
	}
